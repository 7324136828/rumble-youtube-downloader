...
import { connectorBadgeClass, connectorLabel, formatDuration, formatSize, mediaPlayback, loadLikes, saveLikes, setPlayerMode, toggleLike } from '../mediaUtils';
...
export default function WatchPage({ videoId, navigate }) {
  ...
  const updateMedia = useCallback((current) => setItems((previous) => previous?.map((item) => item.id === current.id ? current : item)), []);
      ...
      {error ? <div className="cf-view-state" role="alert"><div className="cf-state-icon"><Icon name="refresh" size={30} /></div><h2>We couldn't load this video</h2><p>{error}</p><button className="btn-primary" onClick={() => setRetry((value) => value + 1)}><Icon name="refresh" size={16} /> Try again</button></div>
        ...
        : <div className="cf-watch-layout">
          <div className="cf-watch-primary">
            {!mediaPlayback(video).audioOnly && <PlaybackWarning video={video} />}
            <CustomVideoPlayer ref={playerRef} key={video.id} src={mediaPlayback(video).src} audioOnly={mediaPlayback(video).audioOnly} poster={video.thumbnail_url || undefined} title={video.title || video.file_name || 'Saved video'} autoPlay variant="watch" formatErrorActions={<MediaConversionButtons video={video} onMediaUpdated={updateMedia} />} onNext={nextVideo ? () => openWatch(nextVideo.id) : undefined} onEnded={() => { rememberPosition(video.id, 0); if (autoplay && nextVideo) openWatch(nextVideo.id); }} onLoadedMetadata={(event) => { try { const time = Number(sessionStorage.getItem(`clipfeed.position.${video.id}`)); const element = event.currentTarget; if (Number.isFinite(time) && time > 0 && time < element.duration - 1) element.currentTime = time; } catch { /* Start at the beginning if storage is unavailable. */ } }} onTimeUpdate={(time) => { const second = Math.floor(time); if (lastSecond.current !== second) { lastSecond.current = second; rememberPosition(video.id, time); } }} />
            ...
            <div className="cf-watch-actions"><button className={likes.has(video.id) ? 'is-liked' : ''} aria-pressed={likes.has(video.id)} onClick={() => setLikes((previous) => toggleLike(previous, video.id))}><Icon name="heart" size={17} />{likes.has(video.id) ? 'Liked' : 'Like'}</button>{video.download_url && <a href={video.download_url} download><Icon name="download" size={17} /> Download</a>}<MediaConversionButtons video={video} formats={video.media_kind === 'audio' ? ['mp4', 'mp3'] : ['mp3']} compact onMediaUpdated={updateMedia} />{video.connector !== 'upload' && video.source_url && <button onClick={copySource}><Icon name="link" size={17} /> Copy link</button>}<button className="cf-watch-delete" onClick={removeVideo} disabled={deleting} aria-label="Delete this video"><Icon name="trash" size={17} />{deleting ? 'Deleting...' : 'Delete'}</button></div>
            <div className="cf-watch-description"><div className="cf-description-heading"><strong>About this video</strong>{video.connector !== 'upload' && video.source_url && <a href={video.source_url} target="_blank" rel="noopener noreferrer">View original <Icon name="external" size={13} /></a>}</div><p className={expanded ? 'expanded' : ''}>{video.description || 'Saved from ' + connectorLabel(video.connector) + ' to your personal library.'}</p>{video.description?.length > 220 && <button onClick={() => setExpanded(!expanded)} aria-expanded={expanded}>{expanded ? 'Show less' : 'Show more'}</button>}</div>
          </div>

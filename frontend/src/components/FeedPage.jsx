            ...
            {videos.map((video, index) => {
              ...
              return <article className="cf-feed-item" key={video.id} ref={(element) => { itemRefs.current[index] = element; }} aria-label={`Video ${index + 1} of ${videos.length}: ${video.title || video.file_name || 'Untitled video'}`} inert={active ? undefined : ''}>
                <div className="cf-feed-stage">
                  {near ? <CustomVideoPlayer ref={(element) => { if (element) videoRefs.current.set(video.id, element); else videoRefs.current.delete(video.id); }} src={mediaPlayback(video).src} audioOnly={mediaPlayback(video).audioOnly} poster={video.thumbnail_url || undefined} title={video.title || video.file_name || 'Saved video'} autoPlay active={active} variant="feed" formatErrorActions={<MediaConversionButtons video={video} onMediaUpdated={updateMedia} />} onEnded={() => { rememberPosition(video.id, 0); if (activeRef.current === index && index < videos.length - 1) goTo(index + 1); }} onLoadedMetadata={(event) => restorePosition(video.id, event.currentTarget)} onTimeUpdate={(time) => { const second = Math.floor(time); if (positionSeconds.current[video.id] !== second) { positionSeconds.current[video.id] = second; rememberPosition(video.id, time); } }}>
